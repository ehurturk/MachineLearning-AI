...
graph = Graph()
# 100 sample seed 1 is the best
X_, y = LinearRegression.generate_dataset(samples=400, features=1, seed=323) # Generating a dataset with 100 samples with only 1 feature with a random seed 1
...
graph.graph_2D(input_x, output_y, 'Dataset', 'Input', 'Output') # Plot the equation
graph.graph_2D(range(len(reg.cost_history_)), reg.cost_history_, 'Graph of Convergence of MSE', 'Iterations', 'MSE') # Plot the convergence of the cost function
